    ...
    async def remove_char(self,msg):
        try:
            ...
            if to_remove == 'spell':
                ...
                query = "UPDATE char_table SET spells = spells - {} WHERE char_id='{}' and campaign_id='{}'".format(
                    args['spell'],char_dat['char_id'],char_dat['campaign_id']
                )
            elif to_remove == 'item':
            ...
            if reply['success']:
                c = dndio_pb2.dndioreply(
                    orig_cmd = 'char remove {}'.format(to_remove),
                    dc_channel=msg.dc_channel,
                    dc_user=msg.user,
                    addtl_data='Successfully removed {} from character sheet'.format(to_remove),
                    err_msg=''
                )
                r = dndio_pb2.charreply(
                    common=c
            ...
            err_msg = await self.proc_err_msg(
                msg,
                'char remove',
                ERR_MSGS['implementation_err']+str(e)
            )
            return err_msg
